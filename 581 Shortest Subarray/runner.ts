...
export function findUnsortedSubarray(nums: number[]): number {
    ...
    for (let i = 0; i < nums.length - 1; i++) {
        ...
        if (foundUnsorted) min = Math.min(min, nums[i+1]);
    }
    foundUnsorted = false;
    for (let i = nums.length - 1; i > 0; i--) {
        if (nums[i] < nums[i - 1]) foundUnsorted = true;
        if (foundUnsorted) max = Math.max(max, nums[i-1]);
    }
...
function findUnsortedSubarray_1(nums: number[]): number {
    // Pointer approach
    let leftPos: number = 0, rightPos: number = 0, tempLeft: number = - 1, tempRight = -1;
